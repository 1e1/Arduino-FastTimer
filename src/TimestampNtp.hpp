...
protected:
    ...
    {
        // parsePacket() returns int and may be negative (error / no packet);
        // cast the size to int so the comparison stays signed (a negative
        // result must not wrap to a huge unsigned and read a phantom packet).
        return this->_udp.parsePacket() >= static_cast<int>(sizeof(NTP_PACKET));
    }
