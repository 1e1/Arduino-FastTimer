...
// Minimal UDP stand-in: parsePacket() returns a settable int (may be negative,
// like the real one on error), read() copies a settable payload.
struct FakeUdp {
    int _avail = 0;
    const uint8_t* _payload = nullptr;
    int parsePacket() { return _avail; }
    int read(uint8_t* buf, size_t len) {
        for (size_t i = 0; i < len; ++i) buf[i] = _payload ? _payload[i] : 0;
        return int(len);
    }
    void flush() {}
};
...
struct TestNtp : public TimestampRFC3339Ntp<U> {
    ...
    void fakePacket(int avail, const uint8_t* payload) {
        this->_udp._avail = avail;
        this->_udp._payload = payload;
    }
};
...
{
    std::printf("listen / _hasResponse\n");

    // A negative parsePacket() (error / no packet) must NOT be treated as a
    // huge unsigned size: listen() must report false, not read a phantom packet.
    {
        TestNtp<FakeUdp> ntp;
        ntp.fakePacket(-1, nullptr);
        CHECK(!ntp.listen());
    }
    // A short packet (fewer bytes than an NTP packet) is also rejected.
    {
        TestNtp<FakeUdp> ntp;
        ntp.fakePacket(20, nullptr);
        CHECK(!ntp.listen());
    }
    // A full packet is accepted and its timestamp decoded (2024-03-01T00:00:00Z).
    {
        const unsigned long s1900 = 1709251200UL + 2208988800UL;
        uint8_t pkt[48] = {0};
        pkt[40] = uint8_t(s1900 >> 24);
        pkt[41] = uint8_t(s1900 >> 16);
        pkt[42] = uint8_t(s1900 >> 8);
        pkt[43] = uint8_t(s1900);
        TestNtp<FakeUdp> ntp;
        ntp.fakePacket(48, pkt);
        CHECK(ntp.listen());
        CHECK(ntp.getTimestampUnix() == 1709251200UL);
    }
}
